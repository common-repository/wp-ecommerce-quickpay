...
class WPEC_QP_Gateway {
		...
		if ( $is_callback ) {
			...
			if ( $payment->is_authorized_callback( $request_body ) ) {
				...
				// Print debug information to logs
					...
					$log->separator();
					$log->add( sprintf( __( 'Transaction failed for #%s.', 'woo-quickpay' ), $json->order_id ) );
					$log->add( sprintf( __( 'Quickpay status code: %s.', 'woo-quickpay' ), $transaction->qp_status_code ) );
					$log->add( sprintf( __( 'Quickpay status message: %s.', 'woo-quickpay' ), $transaction->qp_status_msg ) );
					$log->add( sprintf( __( 'Acquirer status code: %s', 'woo-quickpay' ), $transaction->aq_status_code ) );
					$log->add( sprintf( __( 'Acquirer status message: %s', 'woo-quickpay' ), $transaction->aq_status_msg ) );
					$log->separator();
				}
			} else {
				$log->add( sprintf( __( 'Invalid callback body for order #%s.', 'woo-quickpay' ), $json->order_id ) );
			}
		}
	...
	public static function ajax_manual_transaction_actions() {
		if ( isset( $_REQUEST['quickpay_action'], $_REQUEST['quickpay_transaction_id'], $_REQUEST['quickpay_log_id'] ) ) {
			$param_action         = sanitize_text_field( $_REQUEST['quickpay_action'] );
			$param_transaction_id = sanitize_text_field( $_REQUEST['quickpay_transaction_id'] );
			$param_order_id       = sanitize_text_field( $_REQUEST['quickpay_log_id'] );
